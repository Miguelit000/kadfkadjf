...
export default function Sidebar({ currentPath }) {
  ...
  const [userRole, setUserRole] = useState(null); // Empezamos en null para no dibujar nada incorrecto

  useEffect(() => {
    // Esto se ejecuta única y exclusivamente en el navegador del usuario
    const storedRole = localStorage.getItem('user_role');
    // Si por algún motivo no hay rol, le damos el más bajo (OPERARIO) como escudo
    setUserRole(storedRole || 'OPERARIO');
  }, []);
  ...
  const allMenuItems = [
    ...
    { name: 'Sucursales', path: '/sedes', icon: 'M20 4H4v2h16V4zm1 10v-2l-1-5H4l-1 5v2h1v6h10v-6h4v6h2v-6h1zm-9 4H6v-4h6v4z', roles: ['ADMIN'] }, 
  ...
  // Filtramos la lista, pero si el rol aún es nulo (cargando), devolvemos un arreglo vacío
    ...
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-gray-900 text-white flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          {/* Aquí React pintará los botones reales después de 1 milisegundo de cargar */}
          {menuItems.map((item) => {
